...
def _determine_selfplay_mode(args: argparse.Namespace, mixed_counter: int) -> tuple[str, str, str]:
    """Determine selfplay mode, expected assessor answer, and prompt intent."""
    ...
    return selfplay_mode, expected_assessor, prompt_intent


def _build_injector_prompt_intent(selfplay_mode: str, error_type: str, default_intent: str) -> str:
    """Build prompt intent for injector based on mode."""
    if selfplay_mode == "incorrect":
        if error_type:
            return f"Introduce a {error_type} error while keeping the note realistic."
        return "Introduce a subtle clinical error while keeping the note realistic."
    return default_intent


def _process_generation_attempt(
    generated: str,
    original_note: str,
    ...
    attempt_num: int,
    max_retries: int,
) -> tuple[Optional[str], FilterResult, Optional[Dict], Dict]:
    """Process a single generation attempt: parse, filter, and prepare logging."""
    # Parse output
    parsed_output = None
    ...
    # Apply filters
    filter_result = passes_similarity_filter(
        original_note,
        candidate_note or "",
    ...
    word_edit_info = {}
    if filter_result.passed and candidate_note and HAS_PARSE_CHANGES:
        ...
        if word_diff["total_word_edits"] > max_edits:
            filter_result = FilterResult(passed=False, score_jaccard=filter_result.score_jaccard, reason="too_many_edits")
    ...
    if getattr(args, 'verbose_diff', False) and candidate_note and HAS_PARSE_CHANGES:
        ...
        print(f"\n--- Note {note_id} Attempt {attempt_num}/{max_retries} ---")
        print(diff_output)
        print(f"Filter: {'PASS' if filter_result.passed else 'FAIL'} ({filter_result.reason or 'ok'})")
        print("-" * 60)

    return candidate_note, filter_result, parsed_output, word_edit_info
    ...
    with open(output_path, "a", encoding="utf-8") as out_handle, open(
        ...
        with tqdm(total=len(samples), desc="Self-play", unit="note") as pbar:
            ...
            for record in samples:
                ...
                selfplay_mode, expected_assessor, base_prompt_intent = _determine_selfplay_mode(args, mixed_counter)
                if args.selfplay_mode == "mixed":
                    mixed_counter += 1
                ...
                for attempt_num in range(1, MAX_RETRIES_PER_NOTE + 1):
                    ...
                    # Process generation attempt (parse, filter, log)
                    candidate_note, filter_meta, parsed_output, word_edit_info = _process_generation_attempt(
                        generated, original_note, args, note_id, attempt_num, MAX_RETRIES_PER_NOTE
                    )
                    ...
                    if filter_meta.passed:
                        ...
                        if filter_meta.reason == "too_many_edits":
...
def _setup_models(args: argparse.Namespace):
    """Load tokenizer and model (with optional adapter)."""
...
def _setup_embedder(args: argparse.Namespace):
    """Load embedding model if needed."""
    if args.selfplay or args.disable_embedding:
        return None, None

    embed_device = "cuda" if torch.cuda.is_available() else "cpu"
    return load_embedding_model(args.embedding_model, embed_device)
...
def _build_result_row(
    ...
    score_jaccard: Optional[float],
    score_embedding: Optional[float],
    raw_output: str,
) -> Dict:
    """Build a standardized result row dictionary."""
...
def main() -> None:
    ...
    # Setup models
    tokenizer, model, model_type = _setup_models(args)
    use_qwen_thinking = model_type == MODEL_TYPE_QWEN and args.thinking_budget > 0

    # Setup embedder (if needed)
    embedder_tokenizer, embedder = _setup_embedder(args)
    embed_device = "cuda" if torch.cuda.is_available() else "cpu"
                ...
                if use_qwen_thinking:
                    for meta in metas:
                        ...
                        row = _build_result_row(
                            run_name, scenario, meta, predicted, generated_note,
                            score_jaccard, score_embedding, generated
                        )
