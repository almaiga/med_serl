...
MODEL_PATH="${MODEL_PATH:-FreedomIntelligence/HuatuoGPT-o1-7B}"
...
MAX_SAMPLES="${MAX_SAMPLES:-}"
NO_THINKING="${NO_THINKING:-0}"
PROMPT_CONFIG="${PROMPT_CONFIG:-configs/prompts/detection_localization_prompts.json}"
OUTPUT_DIR="${OUTPUT_DIR:-results/detection}"

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_DIR="${OUTPUT_DIR}/logs"
LOG_FILE="${LOG_DIR}/${MODEL_NAME}_${DATASET}_${TIMESTAMP}.log"
# Screen session name: sanitize slashes and spaces
SCREEN_NAME="detect_$(echo "${MODEL_NAME}" | tr '/: ' '___' | cut -c1-30)"
...
if ! command -v screen &>/dev/null; then
    ...
    if command -v apt-get &>/dev/null; then
        ...
        echo "ERROR: cannot install screen automatically. Please install it manually."
        exit 1
    fi
fi

# ── Guard: don't start duplicate session ─────────────────────────────────────
if screen -list 2>/dev/null | grep -q "${SCREEN_NAME}"; then
    echo "Screen session '${SCREEN_NAME}' already running!"
    echo "  Attach : screen -r ${SCREEN_NAME}"
    echo "  Kill   : screen -X -S ${SCREEN_NAME} quit"
...
# ── Launch in screen ──────────────────────────────────────────────────────────
FULL_CMD="${PYTHON_CMD} 2>&1 | tee -a '${LOG_FILE}'"
