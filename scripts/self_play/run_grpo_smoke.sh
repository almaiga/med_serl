#!/bin/bash
# MedSeRL GRPO Smoke Test (MAGIC-inspired separated training)
#
# Single-turn GRPO, no sglang multi-turn.
# Mixed batches: injector + assessor examples in the same training pass.
# compute_score dispatches rewards by extra_info["role"].
#
# Usage:
#   bash scripts/self_play/run_grpo_smoke.sh
#
# Env overrides:
...
EXPERIMENT_NAME="grpo_smoke_${TIMESTAMP}"
OUTPUT_DIR="outputs/self_play/grpo_smoke_${TIMESTAMP}"
...
echo "=================================================="
echo "MedSeRL GRPO Smoke Test (separated, no multi-turn)"
echo "=================================================="
...
echo ""
echo "=== Step 1: veRL GRPO Training (~${SMOKE_STEPS} steps) ==="
echo ""
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name=medserl-grpo \
...
if [ "$TRAIN_EXIT" -eq 0 ] && [ "$REWARD_HITS" -gt 0 ]; then
    echo "SMOKE TEST PASSED: GRPO training completed with rewards."
elif [ "$TRAIN_EXIT" -eq 0 ]; then
    echo "SMOKE TEST PARTIAL: Training completed but reward calls not found in log."
    echo "Check: grep 'compute_score called' $SMOKE_LOG"
else
