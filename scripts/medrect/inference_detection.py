    ...
    for batch_start in tqdm(range(0, n, batch_size), desc="Inference"):
        ...
        for i, m in enumerate(meta):
            ...
            if is_qwen and use_thinking and input_ids2 is not None:
                if needs_stage2[i]:
                    # ── Position-based split: thinking from pre-stage2, answer from stage2 new tokens
                    pre_ids = input_ids2[i].tolist()
                    pre_ids = [t for t in pre_ids if t != tokenizer.pad_token_id]
                    thinking_ids = pre_ids[input_lens[i]:]
                    thinking = tokenizer.decode(thinking_ids, skip_special_tokens=True).strip()
                    # Answer = only the tokens generated by stage 2
                    answer_ids = outputs[i, stage2_input_len:].tolist()
                    answer_ids = [t for t in answer_ids if t != tokenizer.pad_token_id]
                    content = tokenizer.decode(answer_ids, skip_special_tokens=True).strip()
                else:
                    # ── Completed in stage 1: split from pre-stage2 input (no stage2 garbage)
                    pre_ids = input_ids2[i].tolist()
                    pre_ids = [t for t in pre_ids if t != tokenizer.pad_token_id]
                    out_ids = pre_ids[input_lens[i]:]
                    try:
