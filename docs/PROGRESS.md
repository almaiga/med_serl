...
**step33 MEDEC eval (thinking mode, vLLM, 925 samples, single seed):** acc 0.568, **F1 0.486, recall 0.398**, precision 0.624, sent-acc 0.345 — **below the SFT init** (0.540/0.459/0.402) with everything shifted conservative (flags only 303/925 notes at 51% prevalence). In-game reward was RISING at the same time (−0.49→+0.20): the policy is learning what the game teaches, and the game mix is majority-CORRECT (~23% injector parse-failure games where ground truth is overridden to CORRECT + ~half of valid games benign). Early conservative dip — same dynamic existed under r2 (which dipped early and still reached 0.700), so not yet a verdict. **Decision rule set 2026-07-15: eval step 66 identically after relaunch; recovering → continue; recall still sliding → rebalance the game mix (raise error_injection fraction and/or stop teaching CORRECT on parse-failure games), not the reward.**
